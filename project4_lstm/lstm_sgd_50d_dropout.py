...
# Implement 3 (Dropout LSTM with 50d)
train_loss3 = []
test_loss3 = []
dropout_ratio = 0.2
my_LSTM3 = LSTM_Model(learning_rate=0.1, dp_ratio = dropout_ratio)
for e in range(100):
  for i in range(len(train_x)):
    ...
    my_LSTM3.train(x,y)
  train_loss3.append(sum(my_LSTM3.loss)/len(my_LSTM3.loss))
  my_LSTM3.loss = []
  for i in range(len(test_y)):
    x = word_embedding(test_x[i], w2v50)
    y = one_hot_encoding(test_y[i],5)
    my_LSTM3.test(x,y)
  test_loss3.append(sum(my_LSTM3.loss)/len(my_LSTM3.loss))
  my_LSTM3.loss = []

plt.plot(train_loss3, label='Train loss', color='blue')
plt.plot(test_loss3, label='Test Loss', color='red')
plt.title('Training & Test Loss Over Epochs with LSTM3')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
for i in range(len(test_x)):
  ...
  y = one_hot_encoding(test_y[i],5)
  my_LSTM3.test(x,y)
acc3 = my_LSTM3.get_accuracy()
emojis3 = my_LSTM3.get_emojis()
print(f"Accuracy : {acc3}")
print(f"Emojis : {emojis3}")