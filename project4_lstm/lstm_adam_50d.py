...
# Implement 2 (Adam LSTM with 50d)
train_loss2 = []
test_loss2 = []
beta1 = 0.9
beta2 = 0.999
my_LSTM2 = LSTM_Model(learning_rate = 0.001, optimizer = 'ADAM', beta1 = beta1, beta2 = beta2)
for e in range(60):
  for i in range(len(train_x)):
    ...
    my_LSTM2.train(x,y)
  train_loss2.append(sum(my_LSTM2.loss)/len(my_LSTM2.loss))
  my_LSTM2.loss = []
  for i in range(len(test_y)):
    x = word_embedding(test_x[i], w2v50)
    y = one_hot_encoding(test_y[i],5)
    my_LSTM2.test(x,y)
  test_loss2.append(sum(my_LSTM2.loss)/len(my_LSTM2.loss))
  my_LSTM2.loss = []

plt.plot(train_loss2, label='Train loss', color='blue')
plt.plot(test_loss2, label='Test Loss', color='red')
plt.title('Training & Test Loss Over Epochs with LSTM2')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
for i in range(len(test_x)):
  ...
  y = one_hot_encoding(test_y[i],5)
  my_LSTM2.test(x,y)
acc2 = my_LSTM2.get_accuracy()
emojis2 = my_LSTM2.get_emojis()
print(f"Accuracy : {acc2}")
print(f"Emojis : {emojis2}")