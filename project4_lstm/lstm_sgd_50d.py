...
# Implement 1 (vanilla LSTM with 50d)
epoch = 100
learning_rate = 0.01
train_loss = []
test_loss = []
my_LSTM = LSTM_Model(learning_rate=0.1)
for e in range(epoch):
  for i in range(len(train_x)):
    ...
    my_LSTM.train(x,y)
  train_loss.append(sum(my_LSTM.loss)/len(my_LSTM.loss))
  my_LSTM.loss = []
  for i in range(len(test_y)):
    x = word_embedding(test_x[i], w2v50)
    y = one_hot_encoding(test_y[i],5)
    my_LSTM.test(x,y)
  test_loss.append(sum(my_LSTM.loss)/len(my_LSTM.loss))
  my_LSTM.loss = []

plt.plot(train_loss, label='Train loss', color='blue')
plt.plot(test_loss, label='Test Loss', color='red')
plt.title('Training & Test Loss Over Epochs with LSTM1')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
for i in range(len(test_x)):
  ...
  y = one_hot_encoding(test_y[i],5)
  my_LSTM.test(x,y)
acc = my_LSTM.get_accuracy()
emojis = my_LSTM.get_emojis()
